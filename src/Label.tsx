...
const labelVariants = cva("flex items-center px-2 py-0.5 rounded-[14px] text-sm", {
  variants: {
    variant: {
      regular: "bg-surface-primary/15 text-surface-primary stroke-surface-primary fill-surface-primary",
...
export interface LabelProps extends ComponentPropsWithoutRef<"div">, VariantProps<typeof labelVariants> {
  ...
  ({ className, variant, children, asChild, showInfoIcon, ...props }, ref) => {
    ...
    return (
      <Comp ref={ref} className={labelVariants({ variant, className })} {...props}>
        ...
        {showInfoIcon && <InfoCircle className={"ml-2 w-3.5 h-3.5"} />}
      </Comp>
    );
  },
);
