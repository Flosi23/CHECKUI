...
export interface InputProps
  ...
  label: ReactNode;
  /**
   * An icon to be displayed inside the input
   */
...
const inputTypesSupportingSetSelectionRange: readonly string[] = [
  "text",
  "search",
  "url",
  "tel",
...
/**
 * Input is a component that renders a text input field. It is used to collect user input.
 */
const Input = forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    const handleOnFocus = (event: React.FocusEvent<HTMLInputElement>) => {
      ...
      if (!inputTypesSupportingSetSelectionRange.includes(input.type)) {
        return;
      }
    ...
    const hasIcon = Icon || loading || onClickHelpIcon;
      ...
      <div className={"flex flex-col gap-1"}>
        <div className={"relative"}>
          <input
            ...
            ref={inputRef}
            className={cn(
              `peer w-full rounded-[5px] border pb-2.5 pl-4 pt-6 text-lg font-normal leading-none
            text-on-surface caret-primary shadow-none outline-none
            placeholder-shown:py-4.25 focus:pb-2.5 focus:pt-6
