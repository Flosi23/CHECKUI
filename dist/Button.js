...
export default function Button({ variant = "filled", iconPosition = "left", icon, children, className, ...props }) {
    ...
    const Icon = icon && _jsx("div", { className: "min-h-5 min-w-5", children: icon });
    return (_jsxs("button", { className: cn("relative flex h-12.5 items-center justify-center gap-3 rounded-[3px] px-5 text-xl font-normal shadow-[0_2px_3px_0_rgba(0,0,0,0.10)]", VARIANT_MAP[variant], className), ...props, children: [Icon && iconPosition === "left" && Icon, children, Icon && iconPosition === "right" && Icon] }));
}
